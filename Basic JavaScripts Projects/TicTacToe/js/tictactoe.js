...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        checkWinConditions();
        // This conditions is for changing the activePlayer.
        if (activePlayer === 'X') {
    ...
    function computersTurn() {
        ...
        let pickASquare;
        // This condition allows our wile loop to keep
        // trying if a square is selected already.
        while (!success) {
            // A random number between 0 and 8 is selected
            pickASquare = String(Math.floor(Math.random() * 9));
            // If the random number evaluates returns true,
...
function checkWinConditions() {
    ...
    //  O 1, 4, 7 condition.
    else if (arrayIncludes('1O', '4OX', '7O')) { drawWinLine(304, 50, 304, 558); }
    //  O 2, 5, 8 condition.
